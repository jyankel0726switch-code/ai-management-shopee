...
## Amazon.co.jp地域誤判定時のフォールバック処理(価格取得不可時) 

ASIN直接投入モードで商品ページにアクセスした際、地域誤判定(「アメリカ合衆国」表示等)により価格・在庫情報が完全に取得できない場合、処理を停止せず以下の方式で出力を継続する。

価格欄: 取得できた情報(商品名、画像、JANコード等)は通常通り記載し、価格のみ仮価格「0」として出力する
フラグ: 出力ファイルの該当行(または備考欄)に「要確認：Amazon地域誤判定のため価格未取得」と明記する
在庫・翌日発送確認: 同様に取得不可の場合は「未検証」として扱い、除外はしない
複数商品を一括処理している場合、1件が地域誤判定でも他の商品の処理は継続する
後日、amazon-verify-startupスキル(PC起動時のローカル実行)または手動確認で価格を補完し、フラグを解消する運用とする

## 実行環境に関する重要な前提

本スキルの5.6（Cover imageのフレーム加工）は...
## 実行環境に関する重要な前提
...
- ASINごとにAmazon.co.jpの商品ページ(`https://www.amazon.co.jp/dp/{ASIN}`)を実際にFetchし、
  ...
  - 価格(JPY)。取得できない場合(地域誤判定を含む)は要確認フラグを立てて仮価格「0」として扱う(詳細は上記「Amazon.co.jp地域誤判定時のフォールバック処理」を参照)
...
2. **軸が1〜2つ**の場合:バリエーション出品として処理する。
   ...
   - SKU:**必ず各バリエーションの選択肢(スウォッチ・サムネイル)を実際に1つずつクリックして
     ページ遷移後のURL(またはASIN表示)から個別ASINを取得し、それをそのまま使用する。**
     枝番(`{親ASIN}-01`等)は、全選択肢をクリックしても個別ASINが変わらない(同一ASIN内の
     選択肢のみ)場合の代替手段としてのみ用い、確認の手間を理由に安易に使わない。
   ...
   - Image per Variation:上記でクリックして開いた各バリエーション個別ASINのページから、
     そのバリエーション自身のメイン商品画像URLを取得してこの列に入れる(親商品の画像を
     全行に使い回さない)。画像が取得できなかった場合のみ空欄にする。
   - 価格・在庫・重量はバリエーションごとに個別取得を試み、取得できない項目は親情報を流用
   - バリエーションがある商品も、バリエーションなし商品と**同じ国別出品ファイル**の
     Templateシートに、同じ7行目以降の並びで続けて書き込む(ファイルを分けない)。
     バリエーションのない行はJ〜O列(バリエーション関連列)を空欄のままにする。
...
1. 5で取得したCover画像のURLから画像をダウンロードする。
...
### 5.7 プレオーダー設定
Amazon商品ページの「お届け予定日」を確認する。

- 発送予定日が「明日（JST基準の翌日）」より後の場合 → プレオーダー扱いにする
  - Templateシートの「Pre-order DTS」列（AK列）に固定値「10」を入力する
- 発送予定日が「明日まで」の場合 → 通常出品(AK列は空欄のまま)
- 発送予定日が商品ページから明確に読み取れない場合は、プレオーダー判定を行わず
  AK列を空欄のまま通常出品として処理し、「要確認：発送日未取得」フラグを立てる
...
**通常商品とバリエーション商品は、国ごとに1つの出品ファイルにまとめて書き込む。**
バリエーションのない商品はJ〜O列(バリエーション関連列)を空欄のまま1行で、
バリエーションがある商品(軸1〜2)は同じファイルの続きの行に、バリエーションの数だけ
複数行に分けて書き込む。ファイルを分割しない。
...
- 保存先: なし（チャット添付のみ）
- 国ごとに、その国の商品行(通常商品・バリエーション商品を問わず)のみを含める
...
フレーム加工未実施のため元のAmazon画像URLを使用した件数、
発送日未取得のためプレオーダー判定を行わなかった件数を含む)、
処理できなかった行を報告する。バリエーションのある商品を含めた場合は、
対象商品・国・バリエーション軸数もあわせて明記する。
