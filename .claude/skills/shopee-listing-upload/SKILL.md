...
| U | Cover image | Optional | 5で取得したAmazon商品ページの画像URLのうち1枚目(メイン画像のURL。取得失敗時は空欄) |
| V〜AC | Item Image 1〜8 | Optional | 5で取得した2枚目以降の画像URLを取得順にV列から1列ずつ反映(最大7枚取得のため実質V〜AB列を使用。取得できた枚数分のみ埋め、残りは空欄のまま無理に埋めない) |
...
- 1枚目に取得した画像URLは、従来どおりCover image欄(U列)にテキストとして入力する。
- 2枚目以降に取得した画像URLは、取得順にItem Image 1(V列)、Item Image 2(W列)…と
  1列ずつ順番にテキストとして入力する(最大7枚取得のため、実質V〜AB列(Item Image 1〜7)を使用し、
...
バリエーション軸3以上のため除外した件数、バリエーション検出不十分のため除外した件数を含む)、
処理できなかった行を報告する。

## 絶対に守ること

- Amazon商品ページの説明文をそのまま複製・転載しない(商品説明文は4-2のルールに従いオリジナルで執筆する)。商品画像については、5のルールに従いAmazon商品ページに掲載された画像の**URLを取得してCover image・Item Image欄にテキストとして入力する**(画像ファイル自体のダウンロード・xlsxへの埋め込みは行わない)。
