...
`visibility`を確認)。2026-09-15時点でこのリポジトリは**private**であり、`raw.githubusercontent.com`
はprivateリポジトリの内容を未認証アクセスには返さない(404になる)ことが確認済み。
すなわちprivateのままではCover image欄に入れたURLをShopee側のサーバーが取得できず、
5.6の加工・push自体が徒労になる。private・確認不可のいずれの場合も、1〜4のダウンロード・
加工・pushの実施を**スキップ**し、直ちに6のフォールバック(元のAmazon画像URLを使用し
「要確認：フレーム加工未実施」フラグを立てる)へ進む。加えてサマリーレポート(11)に
「GitHubリポジトリがprivateのため5.6を実行環境の制約としてスキップした」旨を明記し、
リポジトリをpublicにするか、別の画像ホスティング先を用意するかの判断を人に委ねる
(リポジトリの公開設定変更は本スキルからは行わない)。
