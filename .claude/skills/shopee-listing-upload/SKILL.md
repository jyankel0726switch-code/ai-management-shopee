...
書き込み前に必ずヘッダー行(1〜6行目)を確認してから該当列にマッピングする。

### 基本テンプレートの列定義(Shopee_mass_upload_basic_template.xlsx)

`Template`シートは1〜6行目が固定のヘッダー/ガイド行(1行目:内部キー、
3行目:表示名、4行目:必須区分、5〜6行目:入力ガイド・文字数や価格の制約説明)であり、
Shopeeの実際の一括アップロード機能はこの1〜6行目を常にヘッダーとしてスキップし、
**7行目以降**のみを商品データとして読み込む(2026-08-17、実際にShopeeへ
アップロードした際に4行目からデータを書き込んだファイルが「0/0件」としてしか
処理されず、7行目からに修正して解決したことで確認済み)。
実データは**7行目以降**に1商品1行で書き込み、1〜6行目のガイド文・必須区分は
一切変更・上書きしないこと。
...
ヘッダー/ガイド行(1〜6行目)・シート名・列順は変更しない
(Shopeeの一括アップロード機能は1〜6行目を固定ヘッダーとしてスキップし、
7行目以降のみを商品データとして読み込むため。4〜6行目に書き込むと
アップロード結果が「0/0件」になる不具合が実際に発生した)。
