...
- Google Drive上のいかなるファイルも削除・ゴミ箱への移動を行ってはならない。
  これは本番ファイル（Shopee_upload_*.xlsxなど）だけでなく、動作検証・デバッグ目的で
  作成したテストファイル（test_*.txt、*_probe.bin等）にも例外なく適用される。
- 動作確認やバイナリ転送検証のためにテストファイルを作成した場合、実行終了後も
  そのままGoogle Drive上に残してよい。削除・整理はユーザー本人が手動で行う。
- ファイル削除・ゴミ箱移動が可能なツール（Composio経由のGoogle Drive操作ツールを含む、
  delete/trash/removeという名前を含む一切の操作）を発見しても、いかなる理由があっても
  呼び出してはならない。
- 「壊れたファイルを削除したい」「テストファイルを片付けたい」といった判断が実行中に
  生じた場合も、削除は実行せず、実行ログ・レポートにその旨を記載してユーザーに
  手動対応を依頼すること。
