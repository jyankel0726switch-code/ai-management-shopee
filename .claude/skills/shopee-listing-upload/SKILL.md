...
4〜6行目が必須区分・入力ガイド(4行目:Mandatory/Optional等の区分、5行目:項目説明、
6行目:文字数等の入力フォーマット案内)。実データは**7行目以降**に1商品1行で書き込む。
4〜6行目のガイド文は上書きせずそのまま残すこと(同梱の`Upload sample`シートでも
サンプルデータは7行目から始まっており、これがShopee側の想定行位置)。
2026-08-15の運用で4〜6行目にデータを書き込んだ結果、Shopeeの一括アップロードは
「Successful」と表示されるにもかかわらず登録商品数が0件になる不具合が実際に発生した
(4〜6行目はガイド行としてスキップされ、データ行として認識されないため)。
以前の版ではここに「実データは4行目以降に直接書き込む」という誤った記載があったが、
これは誤りだったため訂正した。
...
### 5. 商品画像(仮画像)の生成
商品名・特徴からAI画像生成で仮のカバー画像を作成する。4-3で生成した画像用USPを
...
ヘッダー行(1〜3行目)・ガイド行(4〜6行目)・シート名・シートの並び順・列順は
一切変更しない(シート順序やアクティブタブを変更すると、チャット添付時のプレビューは
見やすくなるが、Shopeeの一括アップロード自体が失敗する不具合を実際に確認済み)。
