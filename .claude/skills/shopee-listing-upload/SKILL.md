...
1. Amazon商品ページのバリエーション選択UIを確認し、バリエーション軸の数を判定する。
   ...
   **注意: ページのテキスト抽出では、Amazonの色・サイズ選択肢がJavaScriptで
   ...
   これらのいずれかに該当し、かつ全ての選択肢を確実に取得できない場合は、
   「バリエーションなし」として自動処理せず、「要確認:バリエーション検出不十分」
   フラグを立てて該当商品をレポートに記載したうえで、その商品は出品ファイルへの
   自動書き込みを行わない(軸3以上の場合と同様の扱いとする)。
...
4. 「要確認:バリエーション検出不十分」と判定した場合も、3.と同様にこの商品は
   自動処理の対象外とし、出品ファイルは生成しない。
