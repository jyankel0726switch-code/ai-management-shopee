...
2. **軸が1〜2つ**の場合:バリエーション出品として処理する。
   ...
   - **バリエーションがある商品は、バリエーションなし商品と同じ出品ファイルに混在させず、
     国ごとに別ファイル(「バリエーション出品用ファイル」。ファイル名は下記「10. 出力」参照)
     へ書き込む。** 通常商品用ファイルにはバリエーション関連列(J〜O列)を使う行を含めない。
...
**通常商品用ファイルとバリエーション出品用ファイルは国ごとに別々のブックとして生成する。**
バリエーションがある商品(軸1〜2)の行は、他の通常商品とは別にコピーしたテンプレートに
書き込み、通常商品用ファイルには含めない。1つのファイル内では、通常商品用なら
全行バリエーション列(J〜O列)を空欄、バリエーション出品用なら全行バリエーション情報を
埋める、という状態を保つ(同一ファイル内で混在させない)。該当国にバリエーション商品が
1件もない場合は、その国のバリエーション出品用ファイルは生成しない。
...
### 10. 出力
- 通常商品(バリエーションなし)用ファイル名: `Shopee_upload_{国コード}_{YYYY-MM-DD}.xlsx`
- バリエーション出品用ファイル名: `Shopee_upload_{国コード}_{YYYY-MM-DD}_variation.xlsx`
  (YYYY-MM-DDはいずれも上記「日付・タイムゾーンに関する重要ルール」に従い**JST基準**の日付とする)
- 保存先: なし（チャット添付のみ）
- 国ごとに、その国の商品行のみを含める。バリエーション商品がある国のみバリエーション出品用ファイルを追加で生成する(該当なしの国では生成しない)
...
処理できなかった行を報告する。バリエーション出品用ファイルを生成した場合は、
対象国・商品数・バリエーション軸数もあわせて明記する。
